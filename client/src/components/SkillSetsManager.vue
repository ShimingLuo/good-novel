...
/**
 * 技能套件管理器
 * 嵌入到 SkillsPane 顶部，提供套件切换、新建、删除功能
 * 包含展开式管理列表，标明内置/自定义状态
 */
import { ref, computed, onMounted, watch } from 'vue';
import { skillSetsApi, type SkillSetMeta, type Skill } from '../api';
import { useWorkspace } from '../stores/workspace';
import BaseDialog from './BaseDialog.vue';

const workspace = useWorkspace();

const sets = ref<SkillSetMeta[]>([]);
const activeSetId = ref('default');
const loading = ref(false);
const showCreate = ref(false);
const showManage = ref(false);
const deleteTarget = ref<SkillSetMeta | null>(null);

// 管理列表中的 Skills
const setSkills = ref<Skill[]>([]);
...
async function loadSets() {
  loading.value = true;
  try {
    const res = await skillSetsApi.list();
    sets.value = res.sets;
...
async function loadSetSkills() {
  loadingSkills.value = true;
  try {
    setSkills.value = await skillSetsApi.listSkills(activeSetId.value);
  } finally {
...
async function switchSet(setId: string) {
  try {
    await skillSetsApi.setActive(setId);
    activeSetId.value = setId;
    emit('change', setId);
    if (showManage.value) await loadSetSkills();
    workspace.addTaskLog('skill-sets', 'success', `已切换到技能套件: ${sets.value.find(s => s.id === setId)?.name || setId}`);
  } catch (e: any) {
    workspace.addTaskLog('skill-sets', 'error', `切换失败: ${e.message}`);
  }
}
...
async function createSet() {
  ...
  try {
    ...
    await loadSets();
    workspace.addTaskLog('skill-sets', 'success', '新技能套件已创建');
  } catch (e: any) {
    workspace.addTaskLog('skill-sets', 'error', `创建失败: ${e.message}`);
  }
}
...
async function confirmDelete() {
  ...
  try {
    await skillSetsApi.delete(deleteTarget.value.id);
    deleteTarget.value = null;
    await loadSets();
    emit('change', activeSetId.value);
    workspace.addTaskLog('skill-sets', 'success', '套件已删除');
  } catch (e: any) {
    workspace.addTaskLog('skill-sets', 'error', `删除失败: ${e.message}`);
  }
}

function toggleManage() {
  showManage.value = !showManage.value;
  if (showManage.value) loadSetSkills();
}
...
<template>
  <div class="sets-manager">
    <div class="sets-header">
      <span class="sets-label">📦 技能套件</span>
    ...
    <Transition name="slide">
      <div v-if="showManage" class="manage-panel">
        <div class="manage-header">
          <h4>「{{ activeSet?.name }}」包含的 Skills</h4>
          <span class="manage-count">共 {{ setSkills.length }} 项</span>
        </div>
        <div v-if="loadingSkills" class="manage-loading">加载中...</div>
        <div v-else class="manage-list">
          <div v-for="skill in setSkills" :key="skill.name" class="manage-item">
            <div class="item-left">
              <span class="item-source" :class="skill.source">
                {{ skill.source === 'builtin' ? '内置' : '自定义' }}
              </span>
              <span class="item-name">{{ skill.displayName || skill.name }}</span>
            </div>
            <span class="item-desc">{{ skill.description?.slice(0, 40) }}{{ (skill.description?.length || 0) > 40 ? '...' : '' }}</span>
          </div>
        </div>
        <div v-if="!loadingSkills && !setSkills.length" class="manage-empty">
          此套件暂无 Skills，请添加或从已有套件克隆。
        </div>
        <div v-if="activeSet?.builtin" class="manage-tip">
          💡 内置套件不可修改。如需自定义，请点击「＋ 新建套件」并从此套件克隆。
        </div>
        <div v-else-if="setSkills.length" class="manage-tip">
          ✏️ 自定义套件：可在上方 Skills 列表中直接编辑各项内容。
        </div>
      </div>
    </Transition>

    <!-- 新建套件弹窗 -->
    <BaseDialog :open="showCreate" title="📦 新建技能套件" @close="showCreate = false">
      <div class="form-stack">
        <div class="field">
          <label>套件 ID（英文标识）</label>
          <input v-model="newSet.id" placeholder="my-skills" />
          <span class="field-hint">只能包含字母、数字、下划线、连字符</span>
        </div>
        <div class="field">
          <label>套件名称</label>
          <input v-model="newSet.name" placeholder="我的技能套件" />
        </div>
        <div class="field">
          <label>描述（可选）</label>
        ...
        <div class="field">
          ...
          <span class="field-hint">克隆会复制源套件的所有 Skills 作为起点，方便在此基础上修改</span>
        </div>
      </div>
      <template #footer>
    ...
    <!-- 删除确认弹窗 -->
    <BaseDialog :open="!!deleteTarget" title="🗑 删除技能套件" @close="deleteTarget = null">
      <div v-if="deleteTarget" class="delete-warning">
        <p>确定要删除套件 <strong>{{ deleteTarget.name }}</strong> 吗？</p>
        <p class="warning-text">⚠️ 此操作会永久删除该套件中的所有自定义 Skills，不可撤销。</p>
